...
export const MAGIC = {
  ...
  firebolt: {
     ...
     * Ослаблен ещё раз (было 0.85 / 2.3) — магия была сильновата.
     */
    baseDamage: 0.7,
    damagePerCharge: 1.95,
    /**
    ...
    cooldown: 1.05,
    /**
     * Обычный огнешар мага вешает на моба «Горение» — урон по времени.
     * `burnSeconds` — длительность; `burnDpsFrac` — урон в секунду как доля от
     * прямого урона попавшего снаряда. Повторное попадание обновляет таймер и
     * берёт больший dps.
     */
    burnSeconds: 3.5,
    burnDpsFrac: 0.22,
