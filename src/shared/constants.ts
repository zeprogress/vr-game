...
export const WORLD = {
  ...
  size: 255, // м, сторона зоны
  playRadius: 170, // м, круглая граница, за которую игрока не пускают (от центра поляны)
  subdivisions: 102, // плотность сетки террейна
  treeCount: 48,
  grassCount: 2000,
...
/**
 * Озеро с водопадом на юго-западе, за старой границей мира (рядом с HUB,
 * который на (-55,-55) с campRadius=30) — см. план «озеро+горы». Гора стоит
 * ЗА озером (чуть дальше от центра мира), её ближний склон спускается прямо
 * в воду — оттуда и падает водопад.
 */
export const LAKE = {
  x: -95,
  z: -95,
...
// Гора (и водопад на её склоне) стоит от озера ровно в стороне заката —
// та же ось (X,Z), что у sunPos в DayTime.ts dayState() (cos(a), sin(a), -0.35)
// при a=PI: направление (-1,-0.35), нормировано. Так с озера в сторону
// заходящего солнца видно и его, и подсвеченный контражуром водопад в одном
// кадре — раньше гора стояла по диагонали (-1,-1), это градусов 25 в сторону
// от настоящего заката, и толком не совпадало.
export const MOUNTAIN = {
  x: -136,
  z: -110,
  /** Радиус, в котором рельеф поднимается к горе (плавный подъём, не стена). */
  radius: 46,
  /** Высота пика над обычным уровнем земли. */
  peakHeight: 34,
} as const;
