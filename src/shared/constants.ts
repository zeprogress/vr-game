...
export const WORLD = {
  // Расширено под озеро на юго-западе (у HUB) — size/playRadius подняты в
  // том же соотношении, что и раньше (1.5×), subdivisions — на тот же
  // коэффициент, что и size, чтобы шаг сетки (а значит и число треугольников
  // на м²) не изменился. treeCount/grassCount/grassRadius НЕ трогаем — это
  // фиксированный бюджет декора вокруг спавна, а не плотность на всю карту.
...
/**
 * Озеро с водопадом на юго-западе, заметно дальше от лагеря (HUB на
 * (-55,-55), campRadius=30), чем в первой версии — см. план «озеро+горы».
 */
/**
 * Пропорции — по присланному топографическому плану («Озеро с водопадом»,
 * масштаб 1:500): озеро 95×70м (длина вдоль горы/лагеря × ширина). rx —
 * поперёк (восток-запад, было 16), rz — вдоль (к горе/от лагеря, было 25).
 * Эллипс, не два раздельных радиуса гуляют сами по себе — terrain.ts и
 * Lake.ts считают его ОДНИМ и тем же способом (см. `lakeEllipseDist` в
 * terrain.ts), иначе дно и видимая вода разъедутся («вода висит в воздухе»).
 */
// Центр/радиусы и высота воды пересчитаны под РЕАЛЬНУЮ впадину из вручную
// слепленного рельефа (src/shared/data/terrainSculpt.json) — раньше это были
// аналитические числа, не привязанные к настоящей форме бассейна.
// x/z сдвинуты на тот же SCULPT_OFFSET, что и terrain.ts (-35,-25) — вся
// гора/озеро/водопад отодвинуты от лагеря целиком, не только сам лагерь.
export const LAKE = {
  x: -149,
  z: -122,
  rx: 49,
  rz: 43,
  // Приподнята и диск расширен (было -3/44/38) — по просьбе, вода не
  // доходила до берегов.
  waterY: -1.5,
  /**
   * Ширина прибрежной отмели ЗА пределами эллипса, где дно поднимается от дна
   * озера к обычному рельефу — вода (диск в Lake.ts) кроется на всю эту
   * ширину тоже, иначе между кромкой воды и настоящим берегом виден провал
   * голой земли («вода висит в воздухе, не доходит до берега»).
   */
  shoreFade: 8,
} as const;

/**
 * Гора (и водопад на её склоне) — строго К ЮГУ от озера, а не в стороне
 * заката: солнце у DayTime.ts dayState() идёт по дуге С ВОСТОКА (чуть южнее
 * востока) ЧЕРЕЗ ЮГ (в зените) НА ЗАПАД (чуть южнее запада) — то есть весь
 * его ход «параллелен» стене, вытянутой с востока на запад, и ничего не
 * загораживает ни восход, ни закат: сама гора стоит в стороне (юг), а не на
 * пути солнца. Водопад тянется вдоль этой стены (запад-восток) и обращён на
 * север, к озеру — низкое солнце что на восходе, что на закате бьёт вдоль
 * него по касательной.
 */
// x/z — реальное положение обрыва в слепленном рельефе (профиль высоты по
// линии x=-114 показывает резкий подъём ровно у z≈-160, до этого — впадина
// озера). radius/peakHeight формы больше не задают — внутри прямоугольника
// лепки terrain.ts берёт verх ваш heightmap (см. SCULPT_BOUNDS), эти два
// поля остались только для «на сколько далеко считать декор мостом горы»
// (trees.ts/rocks.ts) и для формулы направления озеро→гора в Lake.ts.
...
export const BOW = {
  ...
  maxDrawVR: 0.4, // м, полный натяг (разлёт рук; было 0.62 — тянуть далеко было тяжело)
...
export const SWORD_CRIT_MULT = 1.5;
/** Базовая кратность крита посоха (огнешар); ролл «силы крита» добавляется сверху. */
export const STAFF_CRIT_MULT = 2;
...
export const MOB = {
  ...
  leashDistance: 24, // м от точки спавна — дальше моб принудительно бросает погоню и бежит домой
...
export interface EliteMobDef {
  ...
  splitHpFrac?: number;
  splitDmgMul?: number;
  /** Множитель скорости движения осколков (2 — вдвое быстрее родителя). */
...
export const ELITE_MOBS: Record<string, EliteMobDef> = {
  ...
  //
  // На 30% HP не умирает как обычно, а раскалывается на двух "Осколков
  // голема" — вполовину меньше размером, 15% HP родителя, вдвое слабее
  // бьют, но вдвое быстрее двигаются. Раскол сам по себе даёт 3000 xp
  // (вместо обычных при смерти), каждый осколок при своей смерти — ещё по
...
/**
 * Числовые эффекты легендарных аффиксов (см. items.ts WeaponAffix) плюс
 * врождённый поджог мага. crit — Лук охотника, guard — Эгида, storm —
 * Посох бури, vamp — Меч вампира.
 */
export const AFFIX = {
  /**
   * Врождённый поджог мага (огнешар посоха — прямое попадание И всё, что
   * задело АОЕ, см. ZoneSim.ts tickBolt/splashDamage, TowerRoom.heroAttack).
   * Раньше был аффиксом "Пламенного меча" (заменён на "vamp" — "Меч
   * вампира"), урон в секунду считался от урона удара (burnDpsFrac). Теперь
   * — от МАКСИМАЛЬНОГО HP цели (burnHpFrac): иначе горение было почти
   * незаметно на толстых боссах и избыточно на мелких мобах при одном и том
   * же ударе — доля от собственного запаса HP цели ровнее масштабируется.
   */
  fire: { burnSec: 6, burnHpFrac: 0.02 },
...
export const BOT = {
  ...
  raidAddMemory: 4,
  /**
   * «!follow»-телохранитель: сколько секунд бот «помнит», в какого моба
   * ударила цель (для фокус-фаера), даже если та цель — живой игрок без
   * своего runtime-таргета (только последний засчитанный удар).
   */
  guardAssistMemory: 4,
  ...
  /**
   * Через сколько секунд снять бота, если ХОЗЯИН не появлялся: отсчёт стартует
   * либо от выхода из игры с "оставить бота" (ПК/телефон/VR — без условия про
   * чат), либо от последнего настоящего сообщения в чате канала — дальше
   * продлевается ТОЛЬКО чатом (см. onLeave/onChat). Активность самого бота
   * (фарм) не продлевает. Боты переживают рестарт сервера; таймер после
   * рестарта идёт заново.
   */
  ownerAbsentSec: 5 * 60 * 60,
  ...
  infoCooldown: 20,
  /**
   * Раз во столько секунд в чат канала сама всплывает случайная подсказка
   * про команды (см. TIPS в ZoneRoom.ts) — не все читают `!info`. Только
   * пока в мире есть хоть один активный герой (иначе стрим скорее всего не
   * идёт, и подсказка просто зря засоряет чат). Было 8 мин — по просьбе реже.
   */
  tipIntervalSec: 20 * 60,
   ...
   */
  rainRadius: 6.5, // м круга поражения (было 4.5 — расширили АОЕ по просьбе)
...
export const EVENT = {
  ...
  hardTimeout: 5 * 60,
  /**
   * Доли событий по видам (кумулятивно): башня → охота → остальное нашествие.
   * Было 15%/25%/60% — по просьбе уравняли: 1/3 на каждый вид.
   */
  towerChance: 1 / 3,
  huntChance: 1 / 3,
  invasion: {
   ...
   * записываются по !event, сервер запускает их в свою комнату (TowerRoom) по
   * одному. Здесь — только параметры ВНЕШНЕГО окна очереди; числа самих
   * этажей/мобов/боссов живут в `src/shared/tower.ts`.
   */
  tower: {
    /** Сколько ждать новых записей в очередь, прежде чем считать окно закрытым, с. */
    queueIdleClose: 60,
    /** Общий жёсткий предел на всё окно (очередь + все попытки), с. */
    hardTimeout: 40 * 60,
    /** Камера летит к декоративной башне на поляне перед стартом забега, с. */
    approachSec: 2.4,
  },
} as const;
...
export const SKILL = {
  ...
  arrowRain: {
    ...
    hits: 5, // залпов за время действия; каждый бьёт всех в круге уроном одной стрелы героя
    duration: 3, // с, сколько область держится после каста (и мобы в ней пригвождены)
