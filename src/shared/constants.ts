 ...
 */
/**
 * Blockout-пропорции (см. бриф «Mountain Lake» Phase 1): озеро вытянутое,
 * не круг — rx (поперёк, восток-запад) короче rz (вдоль, к горе/от лагеря).
 * Диаметры воды (без отмели): 2*rx=32м (в диапазоне 25-40 из брифа),
 * 2*rz=50м (в диапазоне 40-60). Эллипс, не два раздельных радиуса гуляют
 * сами по себе — terrain.ts и Lake.ts считают его ОДНИМ и тем же способом
 * (см. `lakeEllipseDist` в terrain.ts), иначе дно и видимая вода разъедутся
 * (ровно так уже ловили баг «вода висит в воздухе»).
 */
export const LAKE = {
  x: -117,
  z: -117,
...
export const MOUNTAIN = {
  x: -117,
  // Придвинута ближе к озеру (было -195) — по концепт-арту гора должна сама
  // формировать дальний край озера (водопад падает прямо из неё), а не
  // стоять отдельным далёким конусом с разрывом до кромки.
  z: -155,
  /** Радиус, в котором рельеф поднимается к горе (плавный подъём, не стена). */
  radius: 60,
  /** Высота пика — по брифу «главный склон 25-40м», без старой завышенной. */
  peakHeight: 34,
} as const;
