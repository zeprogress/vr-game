...
export class PlayerController {
  ...
  update(dt: number): void {
    ...
    // tpFollowAmount). Камера просто догоняет спину персонажа.
    const tpFollow = tp ? tpFollowAmount(inp.moveX, inp.moveY) : 0;
    // Персонаж разворачивается лицом к ходу — КРОМЕ заднего сектора стика
    // (±45° от «на себя»): там «лунная походка», не разворачиваемся. Тогда
    // спина остаётся к камере и догонять ей нечего — ровно как при беге
    // вперёд. Никакой отдельной логики для камеры на «назад» не нужно.
    if (tp && moving && inp.moveY > -Math.abs(inp.moveX)) {
...
/**
 * Насколько активен доворот камеры за спину — пропорционально отклонению
 * стика, от мёртвой зоны у центра до максимума РОВНО на полном отклонении.
 * Одна формула на все направления. «Назад» камеру не крутит не из-за особого
 * правила, а потому что персонаж там не разворачивается (лунная походка) —
 * его спина уже к камере, догонять нечего, как и при беге вперёд.
 */
function tpFollowAmount(mx: number, my: number): number {
  ...
  return clamp((reach - dead) / (1 - dead), 0, 1);
}
