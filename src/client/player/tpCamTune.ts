...
export interface TpCamTune {
  ...
  followRate: number;
  /**
   * Мёртвая зона авто-доворота по оси вперёд/назад: |moveY| ≤ этого — доворота
   * нет. Одинаково вперёд и назад. Боковой стик доворачивает независимо.
   */
  followDead: number;
  /** За сколько после мёртвой зоны доворот выходит на полную (по |moveY|). */
  followSpan: number;
...
export const TP_CAM_TUNE: TpCamTune = {
  ...
  followDead: 0.15, // мёртвая зона по оси вперёд/назад — симметрично
  followSpan: 0.35, // и за столько выходит на полный доворот
  lookSens: 1,
};
