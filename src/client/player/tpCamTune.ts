...
export interface TpCamTune {
  ...
  followRate: number;
  /** Мёртвая зона авто-доворота у центра стика (по большей из осей). */
  followDead: number;
  /**
   * Насколько «назад» полностью выключает авто-доворот камеры: при
   * `-moveY ≥ followSpan` (и когда назад тянут сильнее, чем вбок) камера
   * стоит на месте. Меньше — назад доворот пропадает резче.
   */
  followSpan: number;
...
export const TP_CAM_TUNE: TpCamTune = {
  ...
  followSpan: 0.25, // -moveY ≥ этого → доворот камеры выключен (ход назад)
  lookSens: 1,
};
