...
export const CINE_PATHS: CinePath[] = [
  ...
  {
    // Вся поляна одним кадром сверху (сторона зоны 180 м; опущена до ~130 м и
    // наклонена круче вниз — края поляны уходят за кадр, зато ближе к земле). Медленный дрейф и лёгкий наклон, чтобы кадр не был
    // мёртвой картинкой. Всегда открывает эфир после рестарта сервера.
    name: "Вся поляна",
    duration: 5,
    keys: [
      { p: [0, 120, -240], l: [0, 0, -100] },
      { p: [0, 112, -232], l: [0, 0, -101] },
    ],
  },
];
