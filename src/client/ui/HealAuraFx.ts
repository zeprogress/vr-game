...
export class HealAuraFx {
  ...
  update(dt: number): void {
    for (const a of this.pool) {
      ...
      const k = 0.35 + 0.65 * t;
      // Нижний круг ещё прозрачнее (0.22×k → 0.13×k; раньше 0.5×k) — один и тот же эффект и у кастера,
      // и у остальных, и у зрителя (везде рисует этот класс с теми же числами).
      (a.disc.material as StandardMaterial).alpha = 0.13 * k;
      // Купол: было 0.22×k, потом 0.11×k; по просьбе чуть плотнее — 0.16×k.
      (a.dome.material as StandardMaterial).alpha = 0.16 * k;
    }
  }
