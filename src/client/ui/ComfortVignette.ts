...
// Чёрный тоннель. Форма затемнения — буква «C»: сплошное кольцо по краям,
// но с вырезом на НОСОВОЙ стороне возле центра по вертикали. У левого глаза
// вырез справа (обычная C), у правого — зеркально слева. Так носовые части
// двух картинок не складываются в тёмную полосу и не оставляют просветов.
// Переход градиента — средней мягкости.
...
void main() {
  ...
  float r = length(d);
  // Чистый центр сжимается с ростом intensity; переход средней мягкости.
  float inner = mix(1.2, 0.34, clamp(intensity, 0.0, 1.0));
  float tunnel = smoothstep(inner, inner + 0.28, r);

  // Вырез «C» на носовой стороне. nasal>0 — в сторону носа этого глаза
  // (у левого глаза это +x, у правого -x).
  float nasal = -eye * d.x;
  float openH = smoothstep(0.12, 0.5, nasal);           // раскрывается вглубь носовой стороны
  float openV = 1.0 - smoothstep(0.34, 0.72, abs(d.y)); // только у центра по вертикали
