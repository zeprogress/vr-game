 ...
 * гоблина низкий рост — пусть так и будет, естественный разброс. Было 0.78
 * (×1.5 от исходных 0.52), потом 0.62 — просили ещё меньше, вернули к
 * нативному размеру пака (0.52). Подбор вживую: `?botscale=0.45`.
 */
const BOT_RIG_SCALE = (() => {
  const v = Number(new URLSearchParams(location.search).get("botscale"));
  return Number.isFinite(v) && v > 0.1 && v < 4 ? v : 0.52;
})();
...
export class RemoteAvatar implements Hittable {
  ...
  /** Режим этого аватара (vr/flat/…) — камере спектатора нужно решить, прятать ли модель. */
  get playerMode(): PlayerMode | null {
    return this.mode;
  }
  ...
  private async loadBotRig(): Promise<void> {
    ...
    try {
      while (!this.disposed && this.botRigWant > 0 && this.builtRigSkin !== this.botRigWant) {
        ...
        if (!new URLSearchParams(location.search).has("nomerge")) mergeRigSkinned(this.scene, rig);
