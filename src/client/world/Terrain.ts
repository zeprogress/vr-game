...
function grassMaterial(scene: Scene): StandardMaterial {
  ...
  // Один повтор ~16 м (было 30) — тайлим чаще, значит на каждый метр земли
  // приходится больше текселей одного и того же файла (бесплатнее в плане
  // памяти, чем растить саму текстуру): 1024 px / 16 м ≈ 64 текс/м вместо
  // ~34 — вблизи меньше видно "пиксельные" блоки шума. Текстура периодична,
  // поэтому WRAP встык — без шва при любом коэффициенте повтора.
  const tile = WORLD.size / 16;
