...
/**
 * Сколько настоящих источников ночью раздаём ботам (BotLights). Светлое пятно
 * на земле есть у КАЖДОГО бота — оно ничего не стоит шейдеру освещения; а вот
 * PointLight'ы дороги, поэтому их столько и достаются ближайшим к камере.
 */
