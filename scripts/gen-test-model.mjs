/**
 * Тест-модель для пайплайна ассетов (этап 12): небольшой аккуратный постамент —
 * колонна + тонкая плита сверху, спечён в один меш. Материал НАМЕРЕННО
 * «чужой» (глянцевый металл-рафнесс), чтобы проверять перекраску в плоскую
 * палитру.  Запуск:  node scripts/gen-test-model.mjs
 */
...
// [центр Y, ширина XZ, высота] — небольшой постамент ~0.8 м
const TIERS = [
  { cy: 0.36, w: 0.5, h: 0.72 }, // колонна
  { cy: 0.76, w: 0.66, h: 0.09 }, // плита-крышка
];
...
for (const t of TIERS) {
  for (const f of FACES) {
    ...
    for (const [x, y, z] of f.v) {
      pos.push(x * t.w, t.cy + y * t.h, z * t.w);
      nrm.push(...f.n);
    }
